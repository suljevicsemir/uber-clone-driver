...
class _RiderAccountState extends State<RiderAccount> with TickerProviderStateMixin{
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      body: FutureBuilder(
        future: Provider.of<ProfilePicturesProvider>(context, listen: false).getRiderPicture(widget.rider.firebaseId),
        builder: (context, snapshot) {
          if(snapshot.hasError) {
            return Text('Oooops something went wrong');
          }
          if(snapshot.connectionState == ConnectionState.done) {
            File picture = snapshot.data as File;
